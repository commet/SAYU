...
/**
 * APT 타입별 전시 추천을 위한 전시 분석 시스템 V2
 * 새로운 DB 구조: exhibitions_master, exhibitions_translations, venues
 */
class APTExhibitionAnalyzerV2 {
  constructor() {
    // 16개 APT 타입별 프로파일 정의 (이전과 동일)
    this.typeProfiles = {
      // 몽상가 부족 (Dreamers) - LA군
      LAEF: {
        name: '몽환적 방랑자',
  ...
  /**
   * 새로운 DB 구조에서 전시 데이터 가져오기
   */
  async fetchExhibitionsWithNewStructure(limit = 200) {
    try {
      // exhibitions_master와 exhibitions_translations, venues를 조인
      ...
      const formattedData = data.map(exhibition => ({
        id: exhibition.id,
        title: exhibition.exhibitions_translations[0]?.exhibition_title || '',
        description: exhibition.exhibitions_translations[0]?.description || '',
        venue_name: exhibition.exhibitions_translations[0]?.venue_name || 
                   exhibition.venues?.name || 
                   exhibition.venues?.name_en || '',
        venue_type: exhibition.venues?.type || '',
        category: exhibition.genre || exhibition.exhibition_type || '',
        start_date: exhibition.start_date,
        end_date: exhibition.end_date,
        image_url: exhibition.poster_url || exhibition.thumbnail_url,
        artist_names: Array.isArray(exhibition.exhibitions_translations[0]?.artists) 
          ? exhibition.exhibitions_translations[0].artists 
          : (typeof exhibition.exhibitions_translations[0]?.artists === 'string' 
            ? exhibition.exhibitions_translations[0].artists.split(',') 
            : [])
      }));
  ...
  analyzeExhibitionCharacteristics(exhibition) {
    const text = `${exhibition.title || ''} ${exhibition.description || ''} ${exhibition.category || ''} ${exhibition.artist_names?.join(' ') || ''}`.toLowerCase();
    const venue = exhibition.venue_name || '';
    const venueType = exhibition.venue_type || '';
    ...
    const characteristics = {
      ...
      // 환경 특성 (venue type 활용)
      solitude: this.calculateSolitudeByVenueType(venueType, venue),
      social: this.calculateSocialByVenueType(venueType, venue),
      crowded: this.calculateCrowdLevelByVenueType(venueType, venue),
      
      // 체계성
      systematic: this.calculateSystematic(text, venue, venueType),
      ...
      // 교육/문화
      educational: this.calculateEducational(text, venue, venueType),
      cultural: this.calculateCultural(text, venue),
      
      // 접근성/공유
      accessibility: this.calculateAccessibilityByVenueType(venueType, venue),
  ...
  // venue_type을 활용한 개선된 계산 메서드들
  calculateSolitudeByVenueType(venueType, venue) {
    const quietTypes = ['gallery', 'private_gallery', 'artist_studio'];
    const quietVenues = ['간송', '환기', '리움', '호암'];
    
    let score = 0.5;
    if (quietTypes.includes(venueType)) score += 0.3;
    if (quietVenues.some(v => venue.includes(v))) score += 0.2;
    
    return Math.min(1, score);
  }

  calculateSocialByVenueType(venueType, venue) {
    const socialTypes = ['convention_center', 'art_fair', 'public_space'];
    const socialVenues = ['코엑스', '롯데', 'DDP', '아트페어'];
    
    let score = 0.5;
    if (socialTypes.includes(venueType)) score += 0.3;
    if (socialVenues.some(v => venue.includes(v))) score += 0.2;
    
    return Math.min(1, score);
  }

  calculateCrowdLevelByVenueType(venueType, venue) {
    const crowdedTypes = ['museum_national', 'museum_public', 'convention_center'];
    const popularVenues = ['국립', '서울시립', '예술의전당', '롯데'];
    
    let score = 0.4;
    if (crowdedTypes.includes(venueType)) score += 0.3;
    if (popularVenues.some(v => venue.includes(v))) score += 0.2;
    
    return Math.min(1, score);
  }

  calculateSystematic(text, venue, venueType) {
    const keywords = ['체계', '구성', '순서', '구조', '기획', '큐레이션'];
    const systematicTypes = ['museum_national', 'museum_public'];
    const majorVenues = ['국립', '리움', '서울시립'];
    
    let score = this.keywordScore(text, keywords);
    if (systematicTypes.includes(venueType)) score += 0.2;
    if (majorVenues.some(v => venue.includes(v))) score += 0.1;
    
    return Math.min(1, score);
  }

  calculateEducational(text, venue, venueType) {
    const keywords = ['교육', '학습', '워크샵', '강연', '프로그램', '도슨트'];
    const educationalTypes = ['museum_national', 'museum_public', 'cultural_center'];
    const educationalVenues = ['국립', '서울시립', '어린이'];
    
    let score = this.keywordScore(text, keywords);
    if (educationalTypes.includes(venueType)) score += 0.2;
    if (educationalVenues.some(v => venue.includes(v))) score += 0.1;
    
    return Math.min(1, score);
  }

  calculateAccessibilityByVenueType(venueType, venue) {
    const accessibleTypes = ['museum_national', 'museum_public', 'convention_center'];
    const accessibleVenues = ['국립', '서울시립', 'DDP', '예술의전당', '코엑스'];
    
    let score = 0.5;
    if (accessibleTypes.includes(venueType)) score += 0.3;
    if (accessibleVenues.some(v => venue.includes(v))) score += 0.2;
    
    return Math.min(1, score);
  }

  // 기본 특성 계산 메서드들 (이전과 동일)
  ...
  calculateShareability(text, venue) {
    ...
    const venueScore = shareableVenues.some(v => venue.includes(v)) ? 0.4 : 0;
    return Math.min(1, textScore + venueScore);
  }
  ...
  calculateInstagramWorthy(text, venue) {
    ...
    const textScore = this.keywordScore(text, keywords);
    const venueScore = instagramVenues.some(v => venue.includes(v)) ? 0.4 : 0;
    return Math.min(1, textScore + venueScore);
  }
  ...
  async analyzeAllExhibitions() {
    console.log('🎨 SAYU APT 전시 매칭 분석 V2 시작...\n');
    console.log('📌 새로운 DB 구조 사용: exhibitions_master + exhibitions_translations + venues\n');
    
    const exhibitions = await this.fetchExhibitionsWithNewStructure();
    if (exhibitions.length === 0) return;

    // 샘플 데이터 확인
    console.log('📋 샘플 전시 데이터:');
    if (exhibitions[0]) {
      console.log(`  - 제목: ${exhibitions[0].title}`);
      console.log(`  - 장소: ${exhibitions[0].venue_name}`);
      console.log(`  - 설명: ${exhibitions[0].description?.substring(0, 100)}...`);
    }
    console.log('');
    ...
    Object.keys(this.typeProfiles).forEach(aptType => {
      ...
      exhibitions.forEach(exhibition => {
        ...
        if (matchResult.score > 40) { // 40점 이상만 추천
  ...
  printResults(results) {
    ...
    // 4대 부족별로 그룹화하여 출력
    const tribes = {
      '🌙 몽상가 부족': ['LAEF', 'LAEC', 'LAMF', 'LAMC'],
      '🔍 관찰자 부족': ['LREF', 'LREC', 'LRMF', 'LRMC'],
      '🎪 연결자 부족': ['SAEF', 'SAEC', 'SAMF', 'SAMC'],
      '🎓 전달자 부족': ['SREF', 'SREC', 'SRMF', 'SRMC']
    };

    Object.entries(tribes).forEach(([tribeName, types]) => {
      console.log(`\n${tribeName}`);
      console.log('═'.repeat(60));
      
      types.forEach(aptType => {
        const profile = this.typeProfiles[aptType];
        const exhibitions = results[aptType];
        ...
        if (!exhibitions || exhibitions.length === 0) {
          ...
          exhibitions.slice(0, 3).forEach((ex, idx) => {
            console.log(`\n    ${idx + 1}. ${ex.title} [${ex.score}점]`);
            console.log(`       📍 ${ex.venue}`);
            if (ex.keyword_matches.length > 0) {
              console.log(`       🔑 키워드: ${ex.keyword_matches.join(', ')}`);
            }
            if (ex.venue_match) {
              console.log(`       ✨ 선호 장소!`);
            }
    ...
    Object.values(results).forEach(exhibitions => {
      totalRecommendations += exhibitions.length;
    });
    console.log(`  - 총 추천 전시 수: ${totalRecommendations}개`);
    console.log(`  - 평균 추천 수: ${(totalRecommendations / 16).toFixed(1)}개/타입`);
  }
}

// 실행
async function main() {
  const analyzer = new APTExhibitionAnalyzerV2();
  ...
  const fs = require('fs');
  fs.writeFileSync(
    'apt-exhibition-analysis-results-v2.json',
    JSON.stringify(results, null, 2),
    'utf-8'
  );
  console.log('\n💾 분석 결과가 apt-exhibition-analysis-results-v2.json 파일에 저장되었습니다.');
}
...
module.exports = APTExhibitionAnalyzerV2;